...
// https://datatracker.ietf.org/doc/html/rfc2812#section-1
...
void Server::accept_call() {
	...
	std::cout << "Client: " << User::findUser(_client_fd)->getHostname() << " connected with port: "
			  << User::findUser(_client_fd)->getPort() << std::endl;
}
...
bool Server::authenticationUser(int fd, User *user, Command *command) {
	std::cout << "|" << _password << "|" << std::endl;
	std::cout << "|" << command->getMessage() << "|" << std::endl;
	if (user->getStatus() == FAILURE_PASS || user->getStatus() == SUCCESS_AUTH) {
		if (command->getName() == "PASS" && user->getStatus() == FAILURE_PASS) {
			if (_password == command->getMessage()) {
				user->setStatus(SUCCESS_AUTH);
			} else {
				const char *msg = "Error: failure password\nRetry now!\n";
				send(fd, msg, (sizeof(char) * strlen(msg) + 1), 0);
			}
		}

		if (command->getName() == "NICK" && user->getStatus() == SUCCESS_AUTH) {
			if (!user->findUser(command->getMessage())) {
				user->setNick(command->getMessage());
				user->setStatus(REGISTRED_USER);
			} else {
				const char *msg = "Error: user exist with this nick\nRetry now!\n";
				send(fd, msg, (sizeof(char) * strlen(msg) + 1), 0);
			}
		}
		return false;
...
void Server::message(int fd) {
	...
	if (authenticationUser(fd, user, command)) {
		if (command->getName() == "PRIVMSG") {
			User *userRec = user->findUserByNick(command->getParams()[0]);
			if (userRec) {
				sendMsg(command->getMessage(), userRec->getFd());
			}
		}
		...
		if (command->getName() == "QUIT") {
			...
			str.append(" ");
//			std::cout << "|------- " << command->getMessage() << std::endl; //пофиксить
			if (command->getMessage().size() != 0) {
				str.append(command->getMessage());
			} else {
				str.append("bye bye");
			}
			sendMsg(str, user->getFd());
...
void Server::sendMsg(const std::string &message, const int fd) const {
	send(fd, message.c_str(), (sizeof(char) * message.length() + 1), 0);
}
